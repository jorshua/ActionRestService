package com.hackfmi.askthor.config
...
trait Configuration {
  ...
  lazy val servicePort = Try(config.getInt("service.port")).getOrElse(8099)

  /** Database host name/address. */
  lazy val dbHost = Try(config.getString("db.host")).getOrElse("localhost")

  /** Database host port number. */
  lazy val dbPort = Try(config.getInt("db.port")).getOrElse(5432)
